...
import {
  ...
  type ZimpleDynamoDBSingleTableGroupedEvent<
    EVENT_DETAILS extends EventDetail = EventDetail,
    AGGREGATE extends Aggregate = Aggregate,
  > = GroupedEvent<EVENT_DETAILS, AGGREGATE> & {
    eventStorageAdapter: ZimpleDynamoDBSingleTableEventStorageAdapter;
  };
  
  const hasZimpleDynamoDBSingleTableEventStorageAdapter = (
    groupedEvent: GroupedEvent,
  ): groupedEvent is ZimpleDynamoDBSingleTableGroupedEvent =>
    groupedEvent.eventStorageAdapter instanceof
    ZimpleDynamoDBSingleTableEventStorageAdapter;
  ...
  type ParsedGroupedEvent = ZimpleDynamoDBSingleTableGroupedEvent & {
    context: NonNullable<GroupedEvent['context']>;
  };
  
  const parseGroupedEvents = (
    adapter: ZimpleDynamoDBSingleTableEventStorageAdapter,
    ...
    const newGrpupedEvents = groupedEventsInput.map((e) => {
      return {
        ...e,
        eventStorageAdapter: adapter,
      }
    });
  
    console.log('new grouped events',  newGrpupedEvents);
  
  
  
  
    newGrpupedEvents.forEach((groupedEvent, groupedEventIndex) => {
      if (!hasZimpleDynamoDBSingleTableEventStorageAdapter(groupedEvent)) {
        throw new Error(
          `Event group event #${groupedEventIndex} is not connected to a ZimpleDynamoDBSingleTableEventStorageAdapter`,
        );
      }
      ...
      if (
        ...
        timestampInfos = {
          timestamp: groupedEvent.event.timestamp as string,
          groupedEventIndex,
        };
      }
    ...
    constructor({
      ...
      this.pushEvent = async (eventWithOptTimestamp, options) => {
        const event = {
          ...
          timestamp: eventWithOptTimestamp.timestamp == undefined ? new Date().toISOString() : eventWithOptTimestamp.timestamp as string,
        };
      ...
      this.pushEventGroup = async (options, ...groupedEventsInput) => {
        const { groupedEvents, timestamp = new Date().toISOString() } =
          parseGroupedEvents(this , ...groupedEventsInput);
            ...
            new TransactWriteItemsCommand({
              TransactItems: groupedEvents.map(groupedEvent => ({
                Put: groupedEvent.eventStorageAdapter.getPushEventInput(
                  { ...groupedEvent.event, timestamp: groupedEvent.event.timestamp == undefined ? timestamp : groupedEvent.event.timestamp as string },
        ...
        return {
          eventGroup: groupedEvents.map(({ event }) => ({
            event: { ...event, timestamp: event.timestamp == undefined ? timestamp : event.timestamp as string },
          })),
        };
      };
